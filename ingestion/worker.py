...
from infra.metrics import BINS_FLAGGED_GAUGE, SENSOR_FAULTS_TOTAL, TELEMETRY_PACKETS_RECEIVED
            ...
            async with httpx.AsyncClient(timeout=None) as client:
                async with client.stream(
                    ...
                    async for line in response.aiter_lines():
                        ...
                        packets_processed += 1
                        zone = getattr(packet, "zone_id", "unknown") or "unknown"
                        TELEMETRY_PACKETS_RECEIVED.labels(zone=zone).inc()
...
async def _process_packet(packet: TelemetryPacket) -> None:
    ...
    flagged_count = sum(1 for s in _bin_states.values() if s.flagged_for_collection)
    BINS_FLAGGED_GAUGE.set(flagged_count)
