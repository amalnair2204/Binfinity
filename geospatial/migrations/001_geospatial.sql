-- Zones (large geographic areas, one per zone_type)
...
-- Sectors (2x2 grid sub-divisions of zones)
...
-- Dump yards / disposal facilities
...
-- Extend spill_incidents from Phase 2 with bin location snapshot (no-op if table absent)
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'spill_incidents') THEN
